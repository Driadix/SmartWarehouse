...
- `OPEN` — ребро допускается к обычной маршрутизации и исполнению при соблюдении остальных инвариантов топологии и резервирования.
- `CARRIER_ONLY` — ребро предназначено только для вертикального перевозчика и не может использоваться как прямой путь подвижной транспортной единицы.
- `RESTRICTED` — ребро не должно участвовать в базовой маршрутизации по умолчанию и может быть допущено только явной политикой, зафиксированной отдельным архитектурным решением или составом реализации. В текущем базовом составе такая политика не определена, поэтому нормативная конфигурация фазы 1 не должна опираться на `RESTRICTED` для построения номинального маршрута.
