...
| `startedAt` | — |
| `owner` | — |
| `notes` | Текущая задача ещё не назначена. Рекомендуемый старт: `P1-023`. |
...
| 2026-04-09 | `P1-021` | Реализован materializer `StationTransfer` для пассивной станции через readiness и подтверждённый факт передачи. | `WCS` теперь доводит шаттл до узла станции отдельной нижней командой, фиксирует `BoundaryPositionConfirmed` только после прибытия и готовности границы станции, а завершает шаг лишь после явного подтверждения transfer-факта с атомарной публикацией `PayloadCustodyChanged` в `integration.outbox_messages` и `audit.platform_event_journal`. |
| 2026-04-09 | `P1-022` | Реализован materializer `CarrierTransfer` для режима `SHUTTLE_RIDES_HYBRID_LIFT_WITH_PAYLOAD`. | `WCS` теперь материализует межуровневую передачу через повторно вызываемый state-first materializer: он выдаёт `PrepareTransfer` для шаттла и лифта, после подтверждённой посадки публикует `MoveCarrier`, не завершает шаг до подтверждённого выхода шаттла на целевой `TransferPoint`, а при финальном согласованном снимке публикует `CommitTransfer` и канонический `TransferCommitted`, одновременно освобождая резервирование операции. |
...
| `P1-004` | `platform-core/application` | Реализовать внутрипроцессную шину `SubmitExecutionTask` / `CancelExecutionTask` и базовую диспетчеризацию обработчиков. | `DONE` | `P1-002`, `P1-003` | `WES` может передать шаг в `WCS` без общей записи в чужие таблицы. |
...
| `P1-021` | `platform-core/wcs` | Реализовать materializer для `StationTransfer` пассивной станции через readiness и подтверждённый факт передачи. | `DONE` | `P1-018`, `P1-019` | `NodeReached` не завершает шаг без `PayloadCustodyChanged`. |
| `P1-022` | `platform-core/wcs` | Реализовать materializer для `CarrierTransfer` в режиме `SHUTTLE_RIDES_HYBRID_LIFT_WITH_PAYLOAD`. | `DONE` | `P1-018`, `P1-019` | Межуровневая передача отрабатывает через `PrepareTransfer`, `MoveCarrier`, `CommitTransfer` и подтверждённые постусловия. |
...
- После закрытия `P1-022` следующий рекомендуемый шаг — `P1-023`: реализовать обработку канонических фактов `NodeReached`, `TransferCommitted`, `TransferAborted`, `CapabilityChanged`, `FaultRaised`, `FaultCleared`.
