...
- какие аппаратные возможности могут быть доступны на уровне устройства, но ещё не включены в активный режим платформы.
...
- цифровой двойник (`Digital Twin`) и контур имитационного моделирования как обязательные компоненты базового состава.
...
Для фазы 1 принимается более узкое правило:
...
- пассивные `LoadStation` и `UnloadStation`, в которых операция на границе станции не требует команд нижнего интеграционного интерфейса самой станции;
...
- завершение загрузки или выгрузки требует подтверждённого факта изменения физического удержания груза, полученного через `Station/Site Integration Adapter`;
...
- `WES` планирует `ExecutionTask` на макроуровне и фиксирует ресурсы, `transferMode` и топологические точки операции;
- `WCS` материализует этот макрошаг во внутренние фазы исполнения и канонические команды нижнего уровня;
- `WCS` не меняет по собственной инициативе ресурсы, выбранные `WES`, и глобальную цель маршрута; если продолжить исполнение невозможно, он приостанавливает шаг и передаёт перепланирование в `WES`;
...
- для пассивной станции `WCS` не ждёт подтверждений со стороны станции на уровне нижнего интеграционного интерфейса; факт позиционирования подтверждается `NodeReached`, а факт передачи груза и готовность станции поступают через `Station/Site Integration Adapter`.
...
В текущем базовом составе допускается модульный монолит или ограниченное число процессов. Отдельный микросервис на каждое устройство или семейство устройств не является обязательным требованием.
