...
internal static class TopologyConfigurationGuard
{
  ...
  public static string? OptionalNotWhiteSpace(string? value, string paramName)
  {
    if (value is null)
    {
      return null;
    ...
    return NotWhiteSpace(value, paramName);
  }
  ...
  public static ReadOnlyCollection<T> UniqueReadOnlyList<T, TKey>(
      IEnumerable<T>? values,
  ...
  {
    var materialized = (values ?? Enumerable.Empty<T>()).ToArray();
